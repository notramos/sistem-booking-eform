        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {SERVICE_TYPES.map((serviceType) => {
            ...
            const content = (
              ...
              >
                <CardContent className="flex min-h-[112px] flex-col items-center justify-center gap-2 p-2.5 text-center sm:min-h-[132px] sm:gap-2.5 sm:p-4">
                  <div className={`rounded-lg p-2 ${isEnabled ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                    <Icon className="h-5 w-5" />
                  </div>
                  <div className="w-full min-w-0">
                    <div className="flex items-center justify-center gap-1">
                      <h3 className="line-clamp-2 text-xs font-semibold leading-tight text-foreground sm:text-sm">{serviceType.label}</h3>
